...
type Config struct {
	Config      func(s string) error `json:"-"           yaml:"-"           long:"config"      env:"CONFIG_FILE" description:"Path to config file"`
	LogLevel    string               `json:"logLevel"    yaml:"logLevel"    long:"log-level"   env:"LOG_LEVEL"   default:"warn" choice:"trace" choice:"debug" choice:"info" choice:"warn" choice:"error" choice:"fatal" choice:"panic" description:"Log level"` //nolint:staticcheck
	LogFormat   string               `json:"logFormat"   yaml:"logFormat"   long:"log-format"  env:"LOG_FORMAT"  default:"text" choice:"text" choice:"json" choice:"pretty" description:"Log format"`                                                           //nolint:staticcheck
	Port        int                  `json:"port"        yaml:"port"        long:"port"        env:"PORT"        default:"4182" description:"Port to listen on"`
	HeaderName  string               `json:"headerName"  yaml:"headerName"  long:"header-name" env:"HEADER_NAME" default:"Authorization" description:"Name of the authorization header to check"`
	StatusCode  int                  `json:"statusCode"  yaml:"statusCode"  long:"status-code" env:"STATUS_CODE" default:"401" description:"HTTP status code to return on failure"`
...
func (c *Config) parseFlags(args []string) error {
	...
	c.Config = func(s string) error {
		b, err := os.ReadFile(s) //nolint:gosec
		if err != nil {
